...
def load_llava(model_path, model_base, dtype="fp32"):
    ...
    if dtype=="bf16":
        if "llava-interleave" in model_base:
            model = LlavaForConditionalGeneration.from_pretrained(model_path, torch_dtype=torch.bfloat16)
...
class CustomDataset(Dataset):
    def __init__(self, questions, image_folder, tokenizer, image_processor, model_config, processor):
    ...
    def __getitem__(self, index):
        ...
        qs = line["text"]
        image = Image.open(BytesIO(base64.b64decode(line['image']))).convert("RGB")
        conversation = []
        conversation.append(
            {
            "role": "user",
...
def create_data_loader(model_base, questions, image_folder, tokenizer, image_processor, model_config, processor, scienceqa=False, single_pred_prompt=False, batch_size=1, num_workers=16):
...
def eval_model(args):
    ...
    model, processor = load_llava(model_path, args.model_base, dtype="bf16" if args.bf16 else "fp16")
    model.cuda()
    ...
    with open(answers_file, "w") as ans_file:
        data_loader = create_data_loader(args.model_base, questions, args.image_folder, tokenizer, image_processor, model.config, processor)
        if "llava-1.5" in args.model_base or "llava-interleave" in args.model_base:
            for (input_ids, pixel_values, attention_mask), line in tqdm(zip(data_loader, questions), total=len(questions)):
                ...
                JSON_output_results.append(result_item)
    with open(answers_file.replace(".jsonl", "_results.json"), "w") as f:
        json.dump(JSON_output_results, f, indent=4)
